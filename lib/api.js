...
Api.prototype.dnsRecordCached = function(req, res) {
	...
	this.db_records.collection('main').count({_id: /^.*:1$/}, function(count) {
		over(1, count);
	});

	this.db_records.collection('main').count({_id: /^.*:2$/}, function(count) {
		over(2, count);
	});

	this.db_records.collection('main').count({_id: /^.*:5$/}, function(count) {
		over(5, count);
	});

	this.db_records.collection('main').count({_id: /^.*:15$/}, function(count) {
		over(15, count);
	});

	this.db_records.collection('main').count({_id: /^.*:28$/}, function(count) {
		over(28, count);
	});
};
...
Api.prototype.dnsRecordClean = function(req, res) {
	...
	this.db_records.collection('main').remove({_id: exp}, function(r) {
		res.send({success: 1});
	});
};
